...
import { ShieldCheck, RefreshCw, Plus, Edit2, Trash2, X } from "lucide-react";
import DataTable, { type Column } from "@/components/DataTable";

interface WLNumber { id: number; e164: string; memo: string; }

export default function SystemWhitelistPage() {
  const [numbers, setNumbers] = useState<WLNumber[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [showModal, setShowModal] = useState(false);
  const [editing, setEditing] = useState<WLNumber | null>(null);
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ e164:"", memo:"" });

  const fetchNumbers = async () => {
    setLoading(true); setError("");
    try { const r=await fetch("/api/vos/system-limit-e164"); const d=await r.json(); if(d.error)setError(d.error); else setNumbers(d.numbers||[]); } catch { setError("Failed"); }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const r = await fetch("/api/vos/system-limit-e164", { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
      const d = await r.json();
      if (d.error) { setError(d.error); return; }
      setShowModal(false); setEditing(null); fetchNumbers();
    } catch { setError("Failed"); }
    finally { setSaving(false); }
  };

  const handleDelete = async (id:number) => {
    if(!confirm("Remove this number from the system whitelist?"))return;
    try { await fetch(`/api/vos/system-limit-e164?id=${id}`,{method:"DELETE"}); fetchNumbers(); } catch {}
  };

  const openEdit = (n:WLNumber) => { setEditing(n); setForm({ e164:n.e164, memo:n.memo||"" }); setShowModal(true); };
  const openAdd = () => { setEditing(null); setForm({ e164:"", memo:"" }); setShowModal(true); };

  const columns: Column<WLNumber>[] = [
    { key: "e164", label: "E164", cellClassName: "text-surface-50 font-mono font-medium text-xs" },
    { key: "memo", label: "Memo", render: (n) => n.memo || "—", cellClassName: "text-surface-400" },
  ...
  return (<div className="p-6 space-y-6">
    <div className="flex items-center justify-between"><div><h1 className="text-2xl font-bold text-surface-50">System White List</h1><p className="text-surface-400 text-sm mt-1">{numbers.length} whitelisted numbers</p></div>
    ...
    <DataTable columns={columns} data={numbers} searchKey="e164" loading={loading} emptyMessage="No whitelisted numbers" emptyIcon={<ShieldCheck className="w-10 h-10 text-surface-600" />} pageSize={20} />
    ...
    {showModal&&(<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm"><div className="bg-surface-900 border border-surface-700 rounded-2xl w-full max-w-md mx-4">
      <div className="flex items-center justify-between px-6 py-4 border-b border-surface-800"><h2 className="text-lg font-semibold text-surface-50">{editing?"Edit Number":"Add Number"}</h2><button onClick={()=>setShowModal(false)} className="p-1.5 rounded-lg hover:bg-surface-800 text-surface-500 hover:text-surface-50"><X className="w-5 h-5"/></button></div>
      ...
      <div className="px-6 py-4 border-t border-surface-800 flex gap-3"><button onClick={()=>setShowModal(false)} className="flex-1 px-4 py-2 border border-surface-700 text-surface-300 rounded-lg text-sm hover:bg-surface-800">Cancel</button><button onClick={handleSave} disabled={!form.e164||saving} className="flex-1 px-4 py-2 bg-brand-600 hover:bg-brand-700 text-surface-50 rounded-lg text-sm font-medium disabled:opacity-50">{saving?"Saving...":editing?"Update":"Create"}</button></div>
    </div></div>)}
  </div>);
}
